...
def get_market_spot_data():
    """获取全市场实时行情数据（翻页模式，雪球优先，东财备份）"""
    print("   ↳ [Xueqiu] 启动主数据源 (自动翻页获取模式 + 宽进严出)...")
    data_list = list()
    try:
        ...
        while current_page <= max_page:
            ...
            params = {
                ...
                "size": page_size,
                "order": "desc",
                "order_by": "amount", # 按成交额排序，保证活跃股优先
                "exchange": "CN",
                "market": "CN",
                "type": "sha,sza,bja"
            }
            ...
            if res_data.status_code != 200:
                print(f"     ⚠️ 第 {current_page} 页请求失败，状态码: {res_data.status_code}，停止翻页。")
                break
                
            json_data = res_data.json()
            if 'data' not in json_data or 'list' not in json_data['data']:
                print(f"     ⚠️ 第 {current_page} 页数据格式异常或触发拦截，停止翻页。")
                break
                
            raw_list = json_data['data']['list']
            if not raw_list:
                print("     ✅ 已读取到空页，所有页面读取完毕。")
                break
            ...
            print(f"     📄 进度 [{current_page}/{max_page}] 页 | 获取原始数据: {len(raw_list)} 条 ➜ 初筛合格 (仅主板): {page_valid_count} 条")    
            
            current_page += 1
            # 随机休眠防反爬
            time.sleep(random.uniform(0.5, 1.5))
            
        print(f"   ✅[Xueqiu] 翻页数据获取结束: 累计获取 {len(data_list)} 只初筛标的。")
        
        if not data_list:
            raise Exception("抓取到的有效数据为空")
...
def analyze_market_environment():
    ...
    try:
        ...
        print(f"\n   风格判定: {style_desc} | 锚定指数: {anchor_name}")
